...
namespace Battlehub.RTHandles
{
    ...
    public static class RuntimeHandles
    {
        ...
        private static void DoAxes(Vector3 position, Matrix4x4 transform, RuntimeHandleAxis selectedAxis, float big2)
        {
            ...
            x = transform.MultiplyVector(x * big2);
            y = transform.MultiplyVector(y * big2);
            z = transform.MultiplyVector(z * big2);
        ...
        private static void DoAxes(Vector3 position, Matrix4x4 transform, RuntimeHandleAxis selectedAxis)
        {
            ...
            x = transform.MultiplyVector(x * 1.5f);
            y = transform.MultiplyVector(y * 1.5f);
            z = transform.MultiplyVector(z * 1.5f);
        ...
        public static void DoPositionHandle(Vector3 position, Quaternion rotation, float big1, float big2, RuntimeHandleAxis selectedAxis = RuntimeHandleAxis.None)
        {
            ...
            GL.Begin(GL.LINES);
            DoAxes(position, transform, selectedAxis, big2);
            ...
            //Matrix4x4 transforms = Matrix4x4.TRS(position, rotation, new Vector3(screenScale *1.5f, screenScale * 1.5f, screenScale * 1.5f));
            Matrix4x4 transforms = Matrix4x4.TRS(position, rotation, new Vector3(screenScale * big2, screenScale * big2, screenScale * big2));


            Graphics.DrawMeshNow(Arrows, transforms);
            if (selectedAxis == RuntimeHandleAxis.X)
            {
                Graphics.DrawMeshNow(SelectionArrowX, transforms);
            }
            else if (selectedAxis == RuntimeHandleAxis.Y)
            {
                Graphics.DrawMeshNow(SelectionArrowY, transforms);
            }
            else if (selectedAxis == RuntimeHandleAxis.Z)
            {
                Graphics.DrawMeshNow(SelectionArrowZ, transforms);
            }
        }

        public static void DoRotationHandle(Quaternion rotation, Vector3 position, float circleSize, Axis mAxis, RuntimeHandleAxis selectedAxis = RuntimeHandleAxis.None)
        {
            float screenScale = GetScreenScale(position, Camera.current);
            //float radius = 1;
            ...
            GL.Begin(GL.LINES);
            if (mAxis == Axis.xyz || mAxis == Axis.x)
            {
                GL.Color(selectedAxis != RuntimeHandleAxis.X ? XColor : SelectionColor);
                DrawCircle(xTranform, radius);
            }
            if (mAxis == Axis.xyz || mAxis == Axis.y)
            {
                GL.Color(selectedAxis != RuntimeHandleAxis.Y ? YColor : SelectionColor);
                DrawCircle(yTranform, radius);
            }
            if (mAxis == Axis.xyz || mAxis == Axis.z)
            {
                GL.Color(selectedAxis != RuntimeHandleAxis.Z ? ZColor : SelectionColor);
