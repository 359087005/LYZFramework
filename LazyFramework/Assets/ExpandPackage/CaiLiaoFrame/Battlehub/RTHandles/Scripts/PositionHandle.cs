...
namespace Battlehub.RTHandles
{
    public class PositionHandle : BaseHandle
    {
        ...
        [HideInInspector]
        public float axisCenterSize = 0.2f;//中心点三自由度拖拽移动大小。

        [SerializeField, TooltipAttribute("轴长度")]
        public float axisLength = 1.3f;//轴长度
        ...
        protected override bool OnBeginDrag()
        {
            ...
            Matrix4x4 matrix = Matrix4x4.TRS(transform.position, Rotation, new Vector3(scale * axisLength, scale * axisLength, scale * axisLength));
            float s = 0.3f * scale;
        ...
        protected override void OnDrag()
        {
            ...
            if (GetPointOnDragPlane(Input.mousePosition, out point))
            {
                ...
                if(EffectiveGridSize == 0.0)
                {
                    ...
                    transform.GetComponent<CharacterController>().Move(offset * speed);
